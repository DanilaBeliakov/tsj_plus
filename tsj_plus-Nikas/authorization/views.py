...
def sign_up_page(request):
    if request.method == "POST":
        ...
        try:
            ...
            # return HttpResponse(f"<h2>Пользователь успешно зарегистрирован! Теперь можно перейти на страницу авторизации по ссылке ниже: </h2> <br> <a href='/auth'>Ссылка</a>")
...
def auth_page(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            return HttpResponse(f"<h2>Авторизация не удалась, пользователь с такой почтой и паролем не найден!")
    else:
        # обработка GET-запроса для получения страницы регистрации
        userform = AuthForm()
